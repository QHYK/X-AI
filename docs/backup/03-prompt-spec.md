...
- `event_candidate` 和 `source_digest_candidate` 是资格信号，而不代表最终 Routing。
- `Event`: 报道具体的现实事件、决定、公告、数据发布、事故，或重要事件的关键新进展
- `Digest`: 文章有信息价值，但主要属于分析、解释、研究、趋势、人物/公司介绍或一般性信息，不以一个需要进入 Today's Events 的重大现实事件为核心。
- `Long-form`: 重要且值得投入时间完整阅读的长篇内容，包括深度分析、重要观点、调查报道、专题文章或系统性解释。可能来自任何来源/类别。
...
4. Rank remaining Source Digest items separately by Category.
...
+ `event_entities` / `event_entities_zh` ≤ 3. 只保留识别该 Event 最重要的核心实体。只保留识别该 Event 最重要的核心实体。
